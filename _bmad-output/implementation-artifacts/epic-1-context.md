...
skrives om i ettertid. Epicen legger lagringsgrunnlaget alle senere epics står
...
punkt med frist før demonstrasjonen. Status 2026-09-24: 1.1–1.3 er ferdige,
resten står i backlog.
...
- **Tre tilstander skal være entydig skillbare i lageret:** rad med styrke 0 (et
  ...
  på en ikke-børsdag (dagen finnes ikke). Kravet gjelder lageret, ikke en
  skjerm. Enhver senere visning, kommando eller spørring skal bevare skillet.
...
- **Hver story leveres med test som kjører uten nett.** Testsettet telles før og
  etter, og tallet føres i commit-meldingen. Det antas ikke.

**Uavklart i kildene:**
- Hva «inneværende børsdag» betyr en lørdag eller en helligdag, er ikke
  avgjort. Det må avgjøres før 1.6 (åpent punkt 3). Det finnes et forslag, men
  det er ikke vedtatt.
...
- **Funksjonell kjerne, imperativt skall, porter som `typing.Protocol`.**
  Kjernemodulene (`signalberegning`, `markedsoversikt`, `aksjedetalj`, `graf`,
  `meldinger`) importerer ikke `requests`, `sqlite3`, `pathlib` eller `flask`.
  Portmodulen `kursdata.py` skal heller ikke gjøre I/O. I dag gjør den det, og
  det lukkes i 1.5.
- **Én port og én skriver per datasett:** `Kurslager` (med lesesiden
  ...
  `<Datasett>leser` er lesesiden av samme port, `<Noe>kilde` leser bare rådata
  fra fil.
...
- **`Kursrad`** har `dato` (`datetime.date`), `slutt`, `justert_slutt` og
  ...
  sted (`kursrad_fra_eodhd`) og faller aldri tilbake fra `adjusted_close` til
  `close`. Beregning bruker justert kurs. Oversikten viser `close`.
...
- **Rekkefølgen inne i epicen:** 1.1 → 1.2 → 1.3 → 1.4a → 1.4b → 1.4c. Hele
  testsettet kjøres mellom hvert steg. Epicteksten sier at `Kursrad` skal inn i
  samme endring som SQLite-adapteren, men rettelsen fra 2026-09-24 sier at det
  ble en egen endring først (1.2, så 1.3). Rettelsen gjelder.
- **Før 1.4b:** testhjelperne `serie()` i tre testfiler lager ugyldige
  tekstdatoer for serier lengre enn 30 rader. De byttes til `date` +
  `timedelta` i en egen commit før 1.4b. 1.4b kan ikke deles videre.
- **1.5b før 1.6:** alle åtte herdingspunktene (a–h) må være på plass før
  `0002` skrives. Åpent punkt 3 må også være avgjort før 1.6, og åpent punkt 24
  før 1.7.
...
- **Epic 4.3** (SQLite-adapter for `KILogg`) venter på Epic 1. **Epic 5B**
  avhenger av 1.4a (`Kursleser`). Story 9.1 skrives når Epic 1 er ferdig.
